...
object FailedToRetrieveResource {
    ...
    fun createErrorPage(
        ...
        return context.resources.openRawResource(R.raw.server500)
            ...
            .replace(
                "%direct_from_website%",
                context.getString(R.string.ceno_sources_direct_from_website)
            )
            .replace(
                "%using_ceno_network%",
                context.getString(R.string.ceno_sources_via_ceno_network)
            )
            .replace(
                "%from_ceno_cache%",
                context.getString(R.string.ceno_sources_shared_by_ceno_cache)
            )
            .replace("%learn_more%", context.getString(R.string.error_page_learn_more))
            .replace(
                "%error_page_learn_more_header%",
                context.getString(R.string.error_page_learn_more_header)
            )
            .replace(
                "%error_page_learn_more_1%",
                context.getString(R.string.error_page_learn_more_1)
            )
            .replace(
                "%error_page_learn_more_2%",
                context.getString(R.string.error_page_learn_more_2)
            )
            .replace(
                "%error_page_learn_more_3%",
                context.getString(R.string.error_page_learn_more_3)
            )
    }
